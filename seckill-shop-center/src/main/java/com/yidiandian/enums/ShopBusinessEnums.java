...
public enum ShopBusinessEnums {
    ...
    private static final Map<Integer, ShopBusinessEnums> valueLookup = new ConcurrentHashMap<>( values().length );

    static {
        for (ShopBusinessEnums type : EnumSet.allOf( ShopBusinessEnums.class )) {
            valueLookup.put( type.code, type );
        }
    }

    public static ShopBusinessEnums resolve(Integer value) {

        return (value != null ? valueLookup.get( value ) : null);
    }

    public static ShopBusinessEnums fromValue(Integer value) {
        ShopBusinessEnums data = valueLookup.get( value );
        if (data == null) {
    ...
    public static List<Map> typeEnumList() {
        ...
        for (ShopBusinessEnums airlineTypeEnum : ShopBusinessEnums.values()) {
    ...
    ShopBusinessEnums(Integer code, String message) {
        this.code = code;
